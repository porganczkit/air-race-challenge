...
class GameEngine {
  ...
  update(deltaTime, elapsedTime) {
    try {
      // Only update game objects if the game is running
      ...
      // Always animate visual elements, even when the game isn't actively playing
  ...
  checkBridgeCollision() {
    ...
    if (distanceToCenter < collisionThreshold) {
      ...
      // Check vertical position relative to bridge
      if (aircraftY > bridgeY && aircraftY < roadwayY) {
        // Successful navigation under bridge
        // Could add special effects or score bonus here
  ...
  gateCompleted(gate) {
    ...
    if (this.currentGateIndex < this.gates.length) {
      ...
      // All gates completed, finish the race!
      this.finishGame();
    }
    
    console.log(`Gate ${this.currentGateIndex} completed, earned ${gatePoints} points`);
  }
  ...
  setupHUD() {
    ...
    messageElement.innerHTML = 'Press SPACE to Start<br><span style="font-size: 20px">Navigate through all gates to complete the course!</span>';
    ...
    resultElement.innerHTML = `
      ...
      <button id="restart-button" style="padding: 10px 20px; margin-top: 15px; font-size: 18px; cursor: pointer; background-color: #4CAF50; border: none; border-radius: 5px; color: white;">Restart Race</button>
    `;
  ...
  startGame() {
    ...
    // Set the first gate as the target
    if (this.gates.length > 0) {
      this.gates[0].setTarget();
    }
  ...
  finishGame() {
    ...
    const finalTime = this.clock.getElapsedTime() - this.gameStartTime;
    
    // Add time bonus to score
    const timeBonus = Math.max(0, Math.floor(1000 - finalTime * 10));
    ...
    document.getElementById('result-time').textContent = `${finalTime.toFixed(1)}s${isNewBestTime ? ' (New Best!)' : ''}`;
    ...
    document.getElementById('result-best-time').textContent = this.bestTime ? `${this.bestTime.toFixed(1)}s` : 'None';
    ...
    console.log('Game finished with time:', finalTime);
  }
