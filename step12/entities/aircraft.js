...
class Aircraft {
  constructor(inputHandler) {
    // Movement properties
    this.maxSpeed = 90; // Maximum speed (increased by another 50% from 60)
    this.forwardSpeed = 45; // Initial forward speed (increased by another 50% from 30)
  ...
  updateCamera() {
    // Get the aircraft's position
    const aircraftPosition = this.object.position.clone();
    
    // Create a proper chase camera view
    // Forward direction based on aircraft orientation
    const forwardVector = new THREE.Vector3(0, 0, 1).applyQuaternion(this.object.quaternion);
    // Up direction for proper camera orientation
    ...
    const cameraPosition = aircraftPosition.clone();
    cameraPosition.sub(forwardVector.clone().multiplyScalar(this.cameraDistance));
    cameraPosition.add(new THREE.Vector3(0, this.cameraHeight, 0));
    
    // Update camera position
    this.camera.position.copy(cameraPosition);
    
    // Make camera look at a point ahead of the aircraft for better visibility
    const targetPosition = aircraftPosition.clone();
    targetPosition.add(forwardVector.clone().multiplyScalar(15)); // Look ahead
    
    // Set the camera to look at this point
